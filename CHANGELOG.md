...
### Backend performance
- `pending` perf(documents-list): added optional `include_summary_preview` query parameter to `/documents` and wired UI to request summary previews only in card view; this avoids loading/parsing suggestion payloads on normal table view while preserving card preview behavior.
