...
### Backend performance
- `pending` test(helpers): added focused tests for `string_list_json` and `note_ids` helpers, and made `normalize_string_list` ordering deterministic (`(lowercase, original)`) to avoid unstable output ordering.
