...
### Backend performance
- `pending` ux(task-runs): enabled Vue Query `keepPreviousData` in `useTaskRunInspector` so paginated/filter changes keep previous rows visible until refreshed data arrives.
