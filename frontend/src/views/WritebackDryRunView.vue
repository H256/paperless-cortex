  ...
  <section class="space-y-4">
    <div class="flex flex-wrap items-end justify-between gap-3">
      <div>
        ...
        <p class="text-sm text-slate-500 dark:text-slate-400">
          Zeigt nur an, welche Paperless-API-Calls ausgefuehrt wuerden. Es werden keine Daten geschrieben.
        </p>
      </div>
    ...
    <div class="flex flex-wrap items-center gap-2 rounded-lg border border-slate-200 bg-white p-3 text-xs dark:border-slate-800 dark:bg-slate-900">
      ...
      <button
        ...
        @click="runDryRun"
      >
        {{ running ? 'Dry-Run laeuft...' : `Dry-Run fuer ${selectedIds.length} Dokument(e)` }}
      </button>
    </div>
    ...
    <div class="space-y-3">
      ...
      >
        <div class="mb-3 flex items-center justify-between gap-3">
          ...
          >
            {{ item.changed ? `geaendert: ${item.changed_fields.join(', ')}` : 'keine Aenderung' }}
          </span>
        </div>
    ...
    >
      <h3 class="text-sm font-semibold text-slate-900 dark:text-slate-100">Dry-Run Ergebnis</h3>
      <p class="mt-1 text-slate-600 dark:text-slate-300">
        Ausgewaehlt: {{ dryRunResult.docs_selected }}, geaendert: {{ dryRunResult.docs_changed }},
        geplante Calls: {{ dryRunResult.calls.length }}
      </p>
...
const loadPreview = async () => {
  ...
  try {
    ...
    errorMessage.value = err instanceof Error ? err.message : 'Preview konnte nicht geladen werden'
  } finally {
    loading.value = false
  }
}
...
const runDryRun = async () => {
  ...
  try {
    ...
    errorMessage.value = err instanceof Error ? err.message : 'Dry-Run fehlgeschlagen'
  } finally {
    running.value = false
  }
}
