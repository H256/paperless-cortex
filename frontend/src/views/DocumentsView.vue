  ...
  <section>
    <div class="flex items-center justify-between">
      ...
      <div class="flex items-center gap-3">
        <div class="grid grid-cols-4 gap-x-3 gap-y-1 rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs text-slate-600 shadow-sm dark:border-slate-800 dark:bg-slate-900 dark:text-slate-300">
          ...
          <div class="font-semibold text-emerald-700 dark:text-emerald-400">{{ stats.fully_processed }}</div>
        ...
        <div v-if="isProcessing" class="flex items-center gap-3 rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs text-slate-600 shadow-sm dark:border-slate-800 dark:bg-slate-900 dark:text-slate-300">
    ...
    <section class="mt-6 rounded-xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
      ...
      <div class="grid gap-4 md:grid-cols-3 lg:grid-cols-8">
        ...
        <div>
          ...
          <select v-model="selectedCorrespondent" class="mt-1 w-full rounded-lg border border-slate-200 bg-white px-2 py-2 text-sm dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100">
        ...
        <div>
          ...
          <select v-model="selectedTag" class="mt-1 w-full rounded-lg border border-slate-200 bg-white px-2 py-2 text-sm dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100">
            <option value="">All</option>
        ...
        <div>
          ...
          <input type="date" v-model="dateFrom" class="mt-1 w-full rounded-lg border border-slate-200 bg-white px-2 py-2 text-sm dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100" />
        </div>
        <div>
        ...
        <div>
          ...
          <select v-model.number="pageSize" class="mt-1 w-full rounded-lg border border-slate-200 bg-white px-2 py-2 text-sm dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100">
      ...
      <div class="mt-4 flex flex-wrap items-center gap-3 text-sm text-slate-600 dark:text-slate-300">
        <button class="ml-auto inline-flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-700 hover:border-slate-300 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200 dark:hover:border-slate-500" @click="load" title="Reload current list">
          <RefreshCw class="h-4 w-4" />
          Reload
        </button>
      </div>
    ...
    <section class="mt-6 rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
      ...
      <div class="flex items-center justify-between px-6 py-4 text-sm text-slate-600 dark:text-slate-300">
        <button class="rounded-lg border border-slate-200 bg-white px-4 py-2 font-semibold text-slate-700 shadow-sm dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200" :disabled="page <= 1" @click="page -= 1; load()">Prev</button>
        <div class="text-center">
          <div class="text-sm font-semibold text-slate-700 dark:text-slate-200">Page {{ page }} of {{ totalPages }}</div>
          <div class="text-xs text-slate-400 dark:text-slate-500">Last synced: {{ formatDateTime(lastSynced) }}</div>
        </div>
        <button class="rounded-lg border border-slate-200 bg-white px-4 py-2 font-semibold text-slate-700 shadow-sm dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200" :disabled="page >= totalPages" @click="page += 1; load()">Next</button>
      </div>
    </section>
  </section>

    <div v-if="showPreviewModal" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 p-4">
      <div class="w-full max-w-xl rounded-2xl border border-slate-200 bg-white p-6 shadow-xl dark:border-slate-800 dark:bg-slate-900">
        <div class="flex items-center justify-between">
          <div>
            <h3 class="text-lg font-semibold text-slate-900 dark:text-slate-100">Ready to process</h3>
            <p class="text-xs text-slate-500 dark:text-slate-400">Summary of missing intelligence tasks</p>
          </div>
        </div>
        ...
        <div v-else class="mt-4 grid gap-3 sm:grid-cols-2">
          <div class="rounded-lg border border-slate-200 bg-slate-50 p-3 text-sm text-slate-700 dark:border-slate-800 dark:bg-slate-800 dark:text-slate-200">
            ...
            <div class="mt-1 text-lg font-semibold text-slate-900 dark:text-slate-100">{{ processPreview?.docs ?? 0 }}</div>
            <div class="mt-1 text-xs text-slate-500">Total checked</div>
          </div>
          <div class="rounded-lg border border-slate-200 bg-slate-50 p-3 text-sm text-slate-700 dark:border-slate-800 dark:bg-slate-800 dark:text-slate-200">
            <div class="text-xs uppercase text-slate-400">Needs work</div>
            <div class="mt-1 text-lg font-semibold text-slate-900 dark:text-slate-100">{{ processPreview?.missing_docs ?? 0 }}</div>
            <div class="mt-1 text-xs text-slate-500">Documents to process</div>
          </div>
          <div class="rounded-lg border border-slate-200 bg-white p-3 text-xs text-slate-600 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-300">
            Missing vision OCR: <strong class="text-slate-900 dark:text-slate-100">{{ processPreview?.missing_vision_ocr ?? 0 }}</strong>
          </div>
          <div class="rounded-lg border border-slate-200 bg-white p-3 text-xs text-slate-600 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-300">
            Missing embeddings: <strong class="text-slate-900 dark:text-slate-100">{{ processPreview?.missing_embeddings ?? 0 }}</strong>
          </div>
          <div class="rounded-lg border border-slate-200 bg-white p-3 text-xs text-slate-600 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-300">
            Missing vision embeddings: <strong class="text-slate-900 dark:text-slate-100">{{ processPreview?.missing_embeddings_vision ?? 0 }}</strong>
          </div>
          <div class="rounded-lg border border-slate-200 bg-white p-3 text-xs text-slate-600 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-300">
            Missing suggestions (baseline): <strong class="text-slate-900 dark:text-slate-100">{{ processPreview?.missing_suggestions_paperless ?? 0 }}</strong>
          </div>
          <div class="rounded-lg border border-slate-200 bg-white p-3 text-xs text-slate-600 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-300">
            Missing suggestions (vision): <strong class="text-slate-900 dark:text-slate-100">{{ processPreview?.missing_suggestions_vision ?? 0 }}</strong>
          </div>
        </div>
