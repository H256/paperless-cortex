  ...
  <section>
    <div>
      <div class="mb-8 flex flex-wrap items-end justify-between gap-4">
        <div>
          <h2 class="text-3xl font-semibold tracking-tight text-slate-900 dark:text-slate-100">
            Dashboard
          </h2>
          <p class="text-sm text-slate-500 dark:text-slate-400">
            Visual overview of processing status and document mix.
          </p>
        </div>
        <button
      ...
      <div
        v-if="error"
        class="rounded-lg border border-rose-200 bg-rose-50 p-4 text-sm text-rose-700 dark:border-rose-900/40 dark:bg-rose-950/40 dark:text-rose-200"
      >
        {{ error }}
      </div>
      ...
      <template v-else>
          ...
          <div class="lg:col-span-4">
            <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              ...
              >
                ...
                <div class="mt-3 text-3xl font-semibold text-amber-600">
                  {{ stats.unprocessed }}
                </div>
                <div class="mt-1 text-xs text-slate-500 dark:text-slate-400">
              ...
              >
                ...
                <div class="mt-3 text-2xl font-semibold text-indigo-600">
                  {{ stats.embeddings }}
                </div>
              ...
              >
                ...
                <div class="mt-3 text-2xl font-semibold text-purple-500">
                  {{ stats.suggestions }}
                </div>
            ...
            <div
              class="mt-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900"
            >
              ...
              <div class="mt-4 h-3 w-full rounded-full bg-slate-100 dark:bg-slate-800">
                <div
                  class="h-3 rounded-full bg-gradient-to-r from-indigo-500 via-emerald-500 to-sky-400"
        ...
        <div class="mt-6 grid gap-4 lg:grid-cols-2">
              ...
              >
                ...
                <div class="mt-1 h-2 rounded-full bg-slate-100 dark:bg-slate-800">
                  <div
                    ...
                    :style="{ width: barPercent(item.count, topCorrespondentsMax) + '%' }"
                  ></div>
                </div>
              </div>
            </div>
          ...
          >
            ...
            <div
              class="mt-4 max-h-72 space-y-2 overflow-auto pr-2 text-xs text-slate-600 dark:text-slate-300"
            >
              <div
        ...
        <div
          class="mt-10 rounded-3xl border border-slate-200 bg-white/90 p-8 shadow-sm backdrop-blur dark:border-slate-800 dark:bg-slate-900/80"
        >
          <div class="flex items-center justify-between">
            <div>
              <div
            ...
            <div
              class="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white p-1 text-xs font-semibold text-slate-500 shadow-sm dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300"
            >
              <button
                ...
                <div v-for="item in unprocessedByCorrespondent.slice(0, 8)" :key="item.name">
                  ...
                  <div class="mt-1 h-2 rounded-full bg-slate-100 dark:bg-slate-800">
                    <div
                      ...
                      :style="{ width: barPercent(item.count, unprocessedMax) + '%' }"
                    ></div>
                  </div>
                </div>
              </div>
                ...
                <div v-for="item in docTypes.slice(0, 8)" :key="item.name">
                  ...
                  <div class="mt-1 h-2 rounded-full bg-slate-100 dark:bg-slate-800">
                    <div
                      ...
                      :style="{ width: barPercent(item.count, docTypeMax) + '%' }"
                    ></div>
                  </div>
                </div>
              </div>
...
const topCorrespondentsMax = computed(() =>
  Math.max(1, ...topCorrespondents.value.map((item) => item.count)),
)
...
const unprocessedMax = computed(() =>
  Math.max(1, ...unprocessedByCorrespondent.value.map((item) => item.count)),
)
