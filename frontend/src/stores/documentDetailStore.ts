...
export const useDocumentDetailStore = defineStore('documentDetail', {
  state: () => ({
    ...
    pageTextsError: '',
    contentQuality: null as { score: number; reasons: string[]; metrics: Record<string, number> } | null,
  ...
  actions: {
    ...
    async suggestField(id: number, source: 'paperless_ocr' | 'vision_ocr', field: string) {
      ...
      try {
        ...
        const variants = (data as any)?.variants?.parsed?.variants || (data as any)?.variants?.variants || [];
        ...
        this.suggestionVariantError = { ...this.suggestionVariantError, [key]: err?.message ?? 'Failed to generate variants' };
      } finally {
    ...
    async pollVariants(id: number, source: 'paperless_ocr' | 'vision_ocr', field: string) {
      ...
      for (let attempt = 0; attempt < 10; attempt += 1) {
        ...
        try {
          ...
          this.suggestionVariantError = { ...this.suggestionVariantError, [key]: err?.message ?? 'Failed to fetch variants' };
          return;
        }
      }
    },
    async applyVariant(id: number, source: 'paperless_ocr' | 'vision_ocr', field: string, value: any) {
      this.suggestionsLoading = true;
      this.suggestionsError = '';
      try {
