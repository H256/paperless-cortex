...
- `3.4 Reduce Import Complexity` moved further again: [`backend/app/routes/documents_actions.py`](backend/app/routes/documents_actions.py) no longer imports sync helpers from another route module and now calls the extracted sync service seam directly, which removes one of the remaining route-to-route dependencies left after the earlier SRP work.
...
- `2.1 Increase Test Coverage` moved further again: the real Weaviate adapter behavior now has direct regression coverage for collection creation, chunk-vs-centroid upsert routing, centroid-search selection, and chunk-to-centroid retrieve fallback in [`backend/tests/test_weaviate_adapter.py`](backend/tests/test_weaviate_adapter.py), which closes the biggest remaining test gap from the `0.5.0` vector-store feature block.
- CI and release hygiene were re-verified again on top of current `develop`: the full backend CI equivalent still passes locally (`ruff`, `mypy`, `pytest`), the frontend CI equivalent still passes locally (`lint`, `type-check`, `test:coverage`, `build`), `cd backend && uv run python scripts/export_openapi.py` still exports successfully, and the scripted version bump path has now advanced the repo to `0.5.2`.
- `6.2 Caching Strategy` is now in progress: the document dashboard cache has been extracted into [`backend/app/services/documents/dashboard_cache.py`](backend/app/services/documents/dashboard_cache.py) and is now explicitly invalidated by sync and document-mutation routes instead of relying only on the short TTL window.
- `2.1 Increase Test Coverage` moved again alongside that cache work: [`backend/tests/test_documents_routes.py`](backend/tests/test_documents_routes.py) now pins that a cached dashboard payload is invalidated after suggestion deletion, so the operations view sees fresh aggregate counts immediately after a mutating action.
- CI and release hygiene were re-verified again on top of current `develop`: the full backend CI equivalent still passes locally (`ruff`, `mypy`, `pytest`), the frontend CI equivalent still passes locally (`lint`, `type-check`, `test:coverage`, `build`), `cd backend && uv run python scripts/export_openapi.py` still exports successfully, and the scripted version bump path has now advanced the repo to `0.5.3`.
- `6.1 Async/Await Optimization` is now effectively complete for the low-risk backend pass: the blocking `/status/stream` payload build in [`backend/app/routes/status.py`](backend/app/routes/status.py) now runs through `asyncio.to_thread(...)`, so the SSE loop no longer performs DB/HTTP work directly on the event loop.
- `2.1 Increase Test Coverage` moved again alongside that async fix: [`backend/tests/test_status_routes.py`](backend/tests/test_status_routes.py) now directly asserts that `/status/stream` offloads the blocking payload build, and [`backend/tests/test_chat_evidence_routes.py`](backend/tests/test_chat_evidence_routes.py) was hardened to stop depending on real PDF/network behavior during route tests.
- `6.2 Caching Strategy` moved further: [`backend/app/services/documents/document_stats_cache.py`](backend/app/services/documents/document_stats_cache.py) now caches the `/documents/stats` aggregate payload, [`backend/app/routes/status.py`](backend/app/routes/status.py) reuses that cached stats snapshot inside `/status/stream`, and the document-stats cache is now explicitly invalidated by sync and document-mutation routes alongside the existing dashboard cache.
- `2.1 Increase Test Coverage` moved again with that cache follow-up: [`backend/tests/test_documents_routes.py`](backend/tests/test_documents_routes.py) now pins that `/documents/stats` invalidates immediately after suggestion deletion instead of serving stale counts until TTL expiry.
- CI and release hygiene were re-verified again on top of current `develop`: the full backend CI equivalent still passes locally (`ruff`, `mypy`, `pytest`), the frontend CI equivalent still passes locally (`lint`, `type-check`, `test:coverage`, `build`), `cd backend && uv run python scripts/export_openapi.py` still exports successfully, and the scripted version bump path has now advanced the repo to `0.5.9`.
- `6.2 Caching Strategy` moved further again: [`backend/app/services/writeback/writeback_preview_cache.py`](backend/app/services/writeback/writeback_preview_cache.py) now caches `only_changed` writeback candidate discovery and preview payloads by doc-id selection, and [`backend/app/routes/writeback_dryrun.py`](backend/app/routes/writeback_dryrun.py) now reuses those cached payloads across preview, execute-now, dry-run-execute, and writeback-job creation paths.
...
- `2.1 Increase Test Coverage` moved again with that cache slice: [`backend/tests/test_writeback_preview_service.py`](backend/tests/test_writeback_preview_service.py) now directly proves cached candidate/preview payloads remain hot until invalidated and then refresh to the new writeback state.
- CI and release hygiene were re-verified again on top of current `develop`: the full backend CI equivalent still passes locally (`ruff`, `mypy`, `pytest`), the frontend CI equivalent still passes locally (`lint`, `type-check`, `test:coverage`, `build`), `cd backend && uv run python scripts/export_openapi.py` still exports successfully, and the scripted version bump path has now advanced the repo to `0.5.10`.
- `6.2 Caching Strategy` moved further again: [`backend/app/services/documents/documents_list_cache.py`](backend/app/services/documents/documents_list_cache.py) now caches the expensive locally-enriched `/documents` pages by request parameters whenever derived state or review-status filtering is involved, instead of rebuilding those overlays on every identical view refresh.
...
- `2.1 Increase Test Coverage` moved again with that cache slice: [`backend/tests/test_documents_routes.py`](backend/tests/test_documents_routes.py) now proves the cached unreviewed documents page invalidates after `POST /documents/{id}/review/mark`, and [`backend/tests/conftest.py`](backend/tests/conftest.py) now resets the in-memory caches between tests so the new cache layers stay isolated in the backend suite.
- CI and release hygiene were re-verified again on top of current `develop`: the full backend CI equivalent still passes locally (`ruff`, `mypy`, `pytest`), the frontend CI equivalent still passes locally (`lint`, `type-check`, `test:coverage`, `build`), `cd backend && uv run python scripts/export_openapi.py` still exports successfully, and the scripted version bump path has now advanced the repo to `0.5.11`.
- `7. Vector Store Migration Prep` moved again during live Weaviate validation: the adapter now normalizes Weaviate's named-vector response shape and correctly maps similarity `min_score` thresholds onto Weaviate's distance-based search API, so the Similar UI filters and doc-level similarity rebuilds behave consistently after the provider switch.
- `5.3 Error Messages & Observability` gained an operational maintenance follow-up: Operations now exposes a missing-vector audit that scans local embedding metadata against the active vector store and surfaces the exact documents whose active chunk vectors are missing or incomplete after a provider migration.
- UX polish moved again: the global processing badge in the app header now distinguishes queued mixed background work from dedicated embedding-only runs, so the top-level status no longer misleadingly reports `Embeddings` while the queue is actually processing multiple task types.
