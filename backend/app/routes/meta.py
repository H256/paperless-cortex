...
def list_tags(page: int = 1, page_size: int = 50, settings: Settings = Depends(get_settings)):
...
def list_correspondents(
    page: int = 1, page_size: int = 50, settings: Settings = Depends(get_settings)
):
...
def get_document_type(doc_type_id: int, settings: Settings = Depends(get_settings)):
...
def get_correspondent(correspondent_id: int, settings: Settings = Depends(get_settings)):
