...
- Global processing visibility slice: added a header-level activity badge in `App.vue` showing live background state (`Sync x/y`, `Embeddings x/y`, `Queue n`, or `Idle`) with pulsing activity dot and contextual tooltip. This makes processing progress visible across all pages without opening queue/detail views.
- Continue-processing state persistence slice: `ContinueProcessingView` now hydrates/syncs processing options from URL query (`sync`, `strategy`, `limit`), so refresh/back/share preserves the exact processing configuration instead of resetting to defaults.
- Header activity UX slice: global processing badge is now actionable. Clicking it opens `Queue` while processing is active, otherwise opens `Documents`; tooltip text now reflects this behavior for clearer navigation.
- Documents triage visibility slice: review quick-filter chips now display live per-state counts (`unreviewed`, `needs_review`, `reviewed`, `all`) derived from the current visible document set, improving at-a-glance workload awareness during triage.
- Global layout responsiveness slice: refined top header/nav for smaller screens (stacked header rows on mobile, wrapping right-side controls, tighter nav chip spacing, and reduced horizontal padding). This prevents action crowding/overlap while keeping nav, activity badge, and theme controls visible.
- Monitoring shortcut slice: added direct `Queue` and `Logs` buttons to Documents processing toolbar, and mirrored quick `Queue`/`Logs` actions in document detail `Operations` header next to "Continue missing processing" for faster troubleshooting jumps.
- UX correction: removed triage chip counters from `DocumentsQuickControls` because filter-dependent counts were misleading in practice (counts changed with selected filters and reduced signal quality).
- API robustness slice: hardened generated OpenAPI client response parsing by introducing `parseBodySafely(...)` and replacing direct `JSON.parse(body)` calls. Plaintext/non-JSON backend errors (e.g. raw 500 responses) now surface as structured error details instead of throwing `Unexpected token ...` parse crashes in the UI.
- Review-flow acceleration slice: document cards now show a contextual `Review` quick action for items in `needs_review`/local-override state, deep-linking directly to detail `Suggestions` tab (`?tab=suggestions`) to reduce clicks during metadata validation.
- Cross-view triage parity slice: added compact row-level quick actions (`Open`, `Continue`, contextual `Review`) to desktop table status cells, matching card-view fast actions so review/processing flow speed is consistent across list layouts.
- Documents filter UX slice: added reusable DocumentsActiveFiltersStrip with removable chips for active correspondent/tag/date/analysis/review/model filters plus one-click clear-all, and wired it into DocumentsView with page-reset clear handlers for faster triage iterations without opening full filter controls.
- Documents filter panel UX simplification: split filters into primary controls (sort/correspondent/tag/analysis/review) and a collapsible Advanced section (date range/model/page size), with auto-open when any advanced filter is active to keep discoverability while reducing default visual noise.
- Documents list feedback polish: wired documentsLoading into the filters panel reload action so the Reload button now disables and shows spinner/text (Reloading...) during refetch, reducing ambiguous click-retry behavior during slow API responses.
- Documents triage UX slice: added a dedicated DocumentsPresetBar with one-click presets (Unreviewed, Needs review, Not analyzed, Inbox) and wired it to existing list filters in DocumentsView for faster repeatable triage setup.
- Documents search UX slice: added URL-synced quick search (q) across document id/title/correspondent/content, wired through DocumentsFiltersPanel, useVisibleDocuments, and route-state sync; active-filter chips now include removable Search chip, and triage presets clear search to keep preset semantics deterministic.
...
- Backend sync safety slice: when malformed note ids are detected in incoming payload, stale-note removal is skipped for that pass to prevent accidental local note deletion; added regression test covering malformed-id stale-removal guard.
- Documents empty-state UX slice: added context-aware empty-state messaging modes (`filtered`, `running_only`, `empty`) so users get precise guidance when running-only mode is active or when no docs are synced yet.
